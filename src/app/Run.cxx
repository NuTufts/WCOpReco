...
namespace wcopreco{
  ...
  wcopreco::Run::Run(){
      ...
      Config_Params cfg_all ;
      // const Config_Saturation_Merger cfg_SM;
      //create the merger and then get a Merged UBEventWaveform UB_Ev_Merged
      wcopreco::Saturation_Merger *merger = new wcopreco::Saturation_Merger(_UB_Ev_wfm, cfg_all._get_cfg_saturation_merger());
      ...
      //do beam hitfinding
      wcopreco::HitFinder_beam hits_found_beam(merged_beam, kernel_container_v, cfg_all._get_cfg_hitfinder_beam(), cfg_all._get_cfg_deconvolver());
      ...
      wcopreco::Flashes_beam flashfinder_beam( &totPE_v,
                                              ...
                                              decon_vv,
                                              beam_start_time,
                                              cfg_all._get_cfg_flashesbeam(),
                                              cfg_all._get_cfg_opflash());

      flashes_beam = flashfinder_beam.get_beam_flashes();
      // std::cout << "\n\n" << flashes_beam.size() << " Beam Flashes in Event\n";

      // //Create the Hitfinder for COSMICS
      wcopreco::HitFinder_cosmic hits_found(&merged_cosmic, &op_gain, &op_gainerror, cfg_all._get_cfg_hitfinder_cosmic(), cfg_all._get_cfg_cophit());

      //flashes for cosmics
      std::vector<COphitSelection>  hits = hits_found.get_ophits_group();
      wcopreco::Flashes_cosmic flashfinder_cosmic(&hits, cfg_all._get_cfg_opflash());
      ...
      //flash filtering
      wcopreco::FlashFiltering flashesfiltered(&flashes_cosmic, &flashes_beam, cfg_all._get_cfg_flashfiltering());
